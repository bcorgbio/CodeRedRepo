...
Images of about 200 species of Lepidoptera were acquired from the Global Biodiversity Information Facility (GBIF) and outlines were digitized using FIJI. In the subsequent analyses, file names were used in order to separate forewing from hindwing data.
After digitization, a Procrustes transformation was performed in order to translate, rotate, and scale the outlines for better analysis of their shape without factoring in arbitrary wing size.
Elliptical Fourier Analysis (EFA) was performed in order to describe wing shapes.
Principle Component Analysis (PCA) was then performed in order to minimize the number of variables explaining the variance in the data set. PCA data was visualized to determine the distribution of morphoshape of the wings of the different species as well as the amount of variance described by different variables.
...
To determine the evolutionary rates, "RRphylo" package was used to determine the rate of evolution based on wing shape, and any statistically significant shifts in shape evolution in the Lepidoptera's clades were identified.
The rate of the significant evolutionary shift was identified along with the name of clades.
(insert relevant coding procedure here)

The phylogenetic independent contrast was performed on the PC data to separate the original tip data independent of each other.
The PC values of forewing and hindwing data were then compared via linear model and analyzed to see if a statistically significant value existed.
(insert relevant coding procedure here)
...
Liz - Wing tracing, Introduction, Methods
